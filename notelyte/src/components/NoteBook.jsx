...
const ExecutiveNotebook = () => {
  ...
  const handleInputSubmit = (e) => {
    ...
    if (mode === "create") {
      ...
      if (type === "nb") {
        ...
        updated[parentNbId][nbKey].sections[newId] = {
          sectionname: tempInput,
          pages: {},
        };
        toggleSection(newId);
      } else if (type === "pg") {
        const nbKey = Object.keys(updated[parentNbId])[0];
        updated[parentNbId][nbKey].sections[parentSecId].pages[newId] = {
          pagename: tempInput,
          content: "",
        };
        setActivePagePath({
      ...
      if (type === "nb") {
        ...
        const nbKey = Object.keys(updated[parentNbId])[0];
        updated[parentNbId][nbKey].sections[parentSecId].pages[id].pagename =
          tempInput;
      }
    }
  ...
  const handleCopy = (menu) => {
    ...
    if (menu.type === "sec")
      data = JSON.parse(
        JSON.stringify(notebooks[menu.parentNbId][nbKey].sections[menu.id]),
      );
    if (menu.type === "pg")
      data = JSON.parse(
        JSON.stringify(
          notebooks[menu.parentNbId][nbKey].sections[menu.parentSecId].pages[
            menu.id
          ],
        ),
      );
  ...
  const onDrop = (e, target) => {
    ...
    if (item.type === "pg" && target.type === "sec") {
      ...
      const pageData =
        updated[item.parentNbId][srcNbKey].sections[item.parentSecId].pages[
          item.id
        ];

      delete updated[item.parentNbId][srcNbKey].sections[item.parentSecId]
        .pages[item.id];

      updated[target.parentNbId][tarNbKey].sections[target.id].pages[item.id] =
        pageData;
    ...
    /* ---------------- SECTION → NOTEBOOK ---------------- */
    if (item.type === "sec" && target.type === "nb") {
      const srcNbKey = Object.keys(updated[item.parentNbId])[0];
      const tarNbKey = Object.keys(updated[target.id])[0];

      // prevent dropping into same notebook
      if (item.parentNbId === target.id) return;

      const sectionData = updated[item.parentNbId][srcNbKey].sections[item.id];

      // remove from source notebook
      delete updated[item.parentNbId][srcNbKey].sections[item.id];

      // add to target notebook
      updated[target.id][tarNbKey].sections[item.id] = sectionData;
  ...
  const currentPage = activePagePath
    ? notebooks[activePagePath.nbId]?.[
        Object.keys(notebooks[activePagePath.nbId])[0]
      ]?.sections[activePagePath.secId]?.pages[activePagePath.pgId]
    : null;
    ...
    <div className="flex h-screen w-full bg-[#fcfcfc] text-[#1a1a1a] font-['Plus_Jakarta_Sans'] p-6 gap-6">
      ...
      <div className="w-80 bg-white border border-[#ececec] rounded-[32px] flex flex-col shadow-sm overflow-hidden">
        <div className="p-8 flex items-center justify-between border-b bg-gray-50/20">
          ...
          <button
            onClick={() =>
              setInputTarget({ type: "nb", id: "root", mode: "create" })
            }
            className="w-8 h-8 bg-black text-white rounded-full font-bold shadow-lg hover:scale-110 transition-transform"
          >
            +
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {inputTarget.type === "nb" && inputTarget.mode === "create" && (
            <input
              autoFocus
              className="w-full p-3 border-2 border-black rounded-xl text-xs font-bold outline-none"
          ...
          {Object.entries(notebooks).map(([nbId, nbData]) => {
            ...
            const isEditingNb =
              inputTarget.id === nbId && inputTarget.mode === "rename";
            ...
            return (
              <div key={nbId} className="group">
                <div
                  ...
                  onDrop={(e) => onDrop(e, { type: "nb", id: nbId })}
                  onContextMenu={(e) => {
                    e.preventDefault();
                    setContextMenu({
                      x: e.pageX,
                ...
                >
                  {isEditingNb ? (
                    <input
                      autoFocus
                      className="bg-transparent font-bold uppercase text-xs outline-none w-full"
                      value={tempInput}
                      onChange={(e) => setTempInput(e.target.value)}
                      onKeyDown={handleInputSubmit}
                    />
                    ...
                    <span className="text-xs font-black uppercase tracking-tight flex items-center gap-2">
                      <span
                        className={`transition-transform duration-200 ${isNbOpen ? "rotate-90" : ""}`}
                      >
                        ▶
                      </span>{" "}
                      {nbName}
                ...
                {isNbOpen && (
                  <div className="ml-5 pl-3 border-l-2 border-gray-100 mt-1 space-y-2">
                    {inputTarget.type === "sec" &&
                      inputTarget.parentNbId === nbId &&
                      inputTarget.mode === "create" && (
                        <input
                          autoFocus
                          className="w-full p-2 border border-black rounded-lg text-[10px] font-bold"
                          placeholder="SECTION..."
                          value={tempInput}
                          onChange={(e) => setTempInput(e.target.value)}
                          onKeyDown={handleInputSubmit}
                    ...
                    {Object.entries(nbData[nbName].sections).map(
                      ([secId, secData]) => {
                        const isEditingSec =
                          inputTarget.id === secId &&
                          inputTarget.mode === "rename";
                          ...
                          >
                            ...
                            {isSecOpen && (
                              <div className="space-y-1 ml-2">
                                {inputTarget.type === "pg" &&
                                  inputTarget.parentSecId === secId &&
                                  inputTarget.mode === "create" && (
                                    <input
                                      autoFocus
                                      className="w-full p-2 border border-gray-200 rounded text-[10px]"
                                ...
                                {Object.entries(secData.pages).map(
                                  ([pgId, pgData]) => {
                                    const isEditingPg =
                                      inputTarget.id === pgId &&
                                      inputTarget.mode === "rename";
          ...
          <>
            <div className="p-10 border-b flex justify-between items-center bg-gray-50/30">
              ...
              <button
                ...
                className="px-8 py-3 bg-black text-white rounded-full text-xs font-bold uppercase hover:invert transition-all"
              >
            ...
            <div className="flex-1 p-2">
              {isPreview ? (
                <div className="p-12 text-xl font-medium leading-relaxed whitespace-pre-wrap">
                  {currentPage.content || "VOID."}
                </div>
              ) : (
                <textarea
                  ...
                  onChange={(e) => {
                    const updated = { ...notebooks };
                    updated[activePagePath.nbId][
                      Object.keys(updated[activePagePath.nbId])[0]
                    ].sections[activePagePath.secId].pages[
                      activePagePath.pgId
      ...
      {contextMenu && (
        <div
          className="fixed bg-white border border-gray-100 shadow-2xl rounded-2xl py-3 w-56 z-50 text-[11px] font-black uppercase tracking-widest overflow-hidden"
          style={{ top: contextMenu.y, left: contextMenu.x }}
        >
          <div
            className="px-5 py-2 hover:bg-black hover:text-white cursor-pointer"
            onClick={() => {
              setInputTarget({
                type: contextMenu.type === "nb" ? "sec" : "pg",
                id: contextMenu.id,
                mode: "create",
                parentNbId:
                  contextMenu.type === "nb"
                    ? contextMenu.id
                    : contextMenu.parentNbId,
                parentSecId: contextMenu.id,
              });
              setTempInput("");
            }}
          >
          ...
          <div
            className="px-5 py-2 hover:bg-black hover:text-white cursor-pointer"
            onClick={() => {
              setInputTarget({ ...contextMenu, mode: "rename" });
              setTempInput("");
            }}
          >
            Rename
          ...
          <div
            className="px-5 py-2 border-t mt-2 hover:bg-black hover:text-white cursor-pointer"
            onClick={() => handleCopy(contextMenu)}
          >
            Copy
          </div>
          <div
            className="px-5 py-2 hover:bg-black hover:text-white cursor-pointer"
            onClick={() => handlePaste(contextMenu)}
          >
            Paste
          </div>
          <div
            className="px-5 py-2 border-t mt-2 text-red-500 hover:bg-red-500 hover:text-white cursor-pointer"
            onClick={() => executeDelete(contextMenu)}
          >
            Delete
          </div>
        </div>
